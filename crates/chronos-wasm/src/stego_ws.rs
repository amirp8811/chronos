...
impl SteganographicWebSocketEngine {
    ...
    /// Establish decoy + real pipes (prototype bookkeeping only).
    pub fn establish_steganographic_pipes(&self, cover_rate_pct: f64) {
        info!(
            "Establishing {} steganographic channels at cover_rate={:.1}%",
    ...
    pub fn parse_stego_ws_frame<'a>(&self, data: &'a [u8]) -> Result<&'a [u8], StegoFrameError> {
        if data.len() < 2 {
